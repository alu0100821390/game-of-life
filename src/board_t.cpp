...
end_(false)
{
    mesh_.resize(0, vector<cell_t>(0));  /* Se redimensiona la malla a 0x0 */
}
...
end_(false)
{
    mesh_.resize(rows, vector<cell_t>(columns));  /* Se redimensiona la malla */
}
...
void board_t::next_turn (void)
{
    ...
    for (int i=0; i<rows_; i++){  /* Se recorre la malla célula a célula */
        for (int j=0; j<columns_; j++){
    ...
    for (int i=0; i<rows_; i++){  /* Se vuelve a recorrer la malla célula a célula */
        for (int j=0; j<columns_; j++){
